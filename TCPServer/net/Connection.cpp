...
int Connection::asyncConnect(const std::string& ip, const unsigned int port,
		boost::function<void(boost::shared_ptr<Connection>,	const boost::system::error_code& )>& handler) {
...
void Connection::onCheckPing(const boost::system::error_code& ec) {
	...
	if((now-last_ping_).total_milliseconds()>=time_out_millisec_){
		//超时以后,如果是服务端链接,会主动断开链接;如果是客户端链接,则会主动发心跳包
		if('s'==connection_type_){
			std::cout<<"time out! connection stopped"<<std::endl;
			stop();
		}
		else if(NULL!=heart_packet_.get()){
			doWrite(*(heart_packet_.get()));
		}
		return;
	}
	doCheckPing();
